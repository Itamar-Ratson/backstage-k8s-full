...
This post walks through the four subsystems that make up "Backstage core," in the order they depend on each other: the **catalog**, the **scaffolder**, **TechDocs**, and the **plugin architecture** underneath all of it. Instead of toy examples, every claim links to a working open-source platform - a repo that runs Backstage on Kubernetes with Argo CD, real templates, custom scaffolder actions, RBAC, and TechDocs - so you can see each concept in a file that actually runs: [backstage-k8s-full](https://github.com/Itamar-Ratson/backstage-k8s-full). All links are pinned to a specific commit, so they'll still be accurate when the repo moves on.
...
*The repo behind every link in this post: [github.com/Itamar-Ratson/backstage-k8s-full](https://github.com/Itamar-Ratson/backstage-k8s-full) - Backstage on Kubernetes with Argo CD, Envoy Gateway, custom scaffolder actions, RBAC, TechDocs, and the CI to hold it together.*
